...
sandbox. The current directory is bound writable at `/workspace`; everything else on the host is
absent, apart from `/usr`, `/bin`, `/lib` and `/lib64` read-only.
...
| workspace | `--tmpfs /workspace`, one `--bind` per directory, `--remount-ro /workspace` |
...
| workspace | `--mount type=tmpfs,destination=/workspace,tmpfs-mode=0555`, one `-v` per directory, `-v <root>:<root>:ro` per `--ro` |
